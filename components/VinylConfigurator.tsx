...
import {
  ...
  MIN_QTY,
  type ProductId,
  type CutType as PricingCutType,
} from "@/lib/pricing";

// ─── Types ────────────────────────────────────────────────────────────────────

export type MaterialId = "vinyl" | "holographic" | "chrome" | "glitter" | "clear";
type VinylTier = "premium" | "economy";
...
const MATERIALS: { id: MaterialId; label: string; swatch: React.CSSProperties }[] = [
  ...
  { id: "chrome", label: "Chrome", swatch: { background: "linear-gradient(135deg,#c0c0c0 0%,#f8f8f8 40%,#a8a8a8 70%,#e8e8e8 100%)" } },
  { id: "glitter", label: "Glitter", swatch: { background: "linear-gradient(135deg,#ffd700 0%,#ff69b4 40%,#da70d6 70%,#ffd700 100%)" } },
  { id: "clear", label: "Clear", swatch: { background: "linear-gradient(160deg,rgba(255,255,255,0.16) 0%,rgba(255,255,255,0.04) 100%)", border: "1px dashed rgba(255,255,255,0.28)" } },
];

const MATERIAL_DESC: Record<MaterialId, string> = {
  vinyl: "Built for everyday durability. Printed on high-quality American vinyl — UV resistant and dishwasher safe.",
  holographic: "Shifts color and catches the light. Stand out with a shimmer that's waterproof, UV-resistant, and scratch-resistant.",
  chrome: "Features a brilliant mirror-like metallic finish. Printed on premium waterproof, UV-resistant material.",
  glitter: "Sparkles with an eye-catching glitter finish — full-color printing on waterproof, UV-resistant material.",
  clear: "See-through stickers that blend seamlessly into any surface. Waterproof and UV-resistant.",
};

const VINYL_TIER_NOTE: Record<VinylTier, string> = {
  premium: "Our most durable vinyl — UV resistant, dishwasher safe, laminated for extra protection.",
  economy: "Affordable indoor stickers with vibrant color and no laminate.",
};
...
function shapeImage(material: MaterialId, shape: ShapeId, cut: CutType): string {
  return `/shapes/${material}-${shape}-${cut}.png`;
}

function productIdFor(material: MaterialId, tier: VinylTier): ProductId {
  if (material === "vinyl") return tier === "economy" ? "economy-vinyl" : "premium-vinyl";
  return material;
}
...
export default function VinylConfigurator({ initialMaterial = "vinyl" }: { initialMaterial?: MaterialId }) {
  const [material, setMaterial] = useState<MaterialId>(initialMaterial);
  const [vinylTier, setVinylTier] = useState<VinylTier>("premium");
  ...
  const product = productIdFor(material, vinylTier);
  ...
  const { perUnit: activePerUnit, total: activeTotal, savings: activeDiscount } = priceFor(activeQty);
  ...
  function handleAddToCart() {
    ...
    const materialTag = material === "vinyl" ? `${vinylTier === "economy" ? "Economy" : "Premium"} Vinyl` : materialLabel;
  ...
  return (
    <>
      <style>{`
        @keyframes sc-float { 0%,100% { transform: translateY(0px); } 50% { transform: translateY(-10px); } }
        @keyframes sc-shimmer { 0% { transform: translateX(-150%) skewX(-20deg); } 100% { transform: translateX(250%) skewX(-20deg); } }
        @keyframes sc-shine { 0% { left: -80%; } 100% { left: 130%; } }
        .sc-card { transition: transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease; }
        .sc-card:hover { transform: translateY(-3px); box-shadow: 0 12px 40px rgba(0,0,0,0.4); }
        .sc-opt { position: relative; overflow: hidden; transition: all 0.22s cubic-bezier(0.4,0,0.2,1); }
        .sc-opt::before {
        ...
        .sc-cta-active::after {
          ...
          animation: sc-shine 2.8s ease-in-out infinite; pointer-events: none;
        }
        .sc-input:focus { outline: none; border-color: rgba(${ACCENT_RGB}, 0.55) !important; box-shadow: 0 0 0 3px rgba(${ACCENT_RGB}, 0.12); }
        .sc-textarea:focus { outline: none; border-color: rgba(${ACCENT_RGB}, 0.4) !important; }
      `}</style>
      ...
      <div className="min-h-screen pb-28 overflow-x-hidden" style={{ background: "#040410" }}>
        <div className="max-w-400 mx-auto px-4">
          ...
          >
            ...
            <div className="relative z-10 px-8 py-7">
              <p style={{
                ...
                fontFamily: "var(--font-orbitron)", color: `rgba(${ACCENT_RGB},0.75)`, marginBottom: "0.5rem",
              }}>Custom Stickers</p>
              <h1 style={{
                fontFamily: "var(--font-orbitron)", fontWeight: 900,
                fontSize: "clamp(1.75rem,3.5vw,2.75rem)", lineHeight: 1.1,
          ...
          {/* ── Select Material ── */}
          <div className="sc-card rounded-[20px] p-5 mt-4 relative overflow-hidden"
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${MATERIALS.length}, 1fr)`, gap: "8px" }}>
              {MATERIALS.map((m) => {
                ...
                return (
                  <button key={m.id} className="sc-opt" onClick={() => setMaterial(m.id)} style={{
                    ...
                    boxShadow: sel ? `0 0 20px rgba(${ACCENT_RGB},0.25)` : "none",
                  }}>
                    <div style={{ width: "100%", height: "40px", borderRadius: "8px", ...m.swatch }} />
                    <span style={{ fontSize: "10px", fontWeight: 700, fontFamily: "var(--font-orbitron)", color: sel ? "white" : "rgba(255,255,255,0.5)" }}>
                      {m.label}
                    </span>
                  </button>
                );
            ...
            <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.45)", lineHeight: 1.7, marginTop: "12px" }}>
              ...
              <div style={{ marginTop: "14px" }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    left: vinylTier === "premium" ? "4px" : "calc(50% + 2px)",
                    width: "calc(50% - 6px)",
                    background: `linear-gradient(135deg,${ACCENT}cc,${ACCENT})`,
                    borderRadius: "8px", transition: "left 0.28s cubic-bezier(0.34,1.56,0.64,1)",
                  }} />
                  {(["premium", "economy"] as VinylTier[]).map((t) => (
                    <button key={t} onClick={() => setVinylTier(t)} style={{
                      ...
                      fontSize: "10px", fontWeight: 700, fontFamily: "var(--font-orbitron)", textTransform: "capitalize",
                      color: vinylTier === t ? "white" : "rgba(255,255,255,0.4)",
                    }}>
                      {t}
                    </button>
                  ))}
                ...
                <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)", marginTop: "8px" }}>
                  {VINYL_TIER_NOTE[vinylTier]}
                </p>
              </div>
            )}
          </div>

          {/* ── 4-column grid: Shape / Size / Finish / Quantity ── */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-4">
            ...
            <div className="sc-card rounded-[20px] p-5 flex flex-col gap-3.5 relative overflow-hidden"
              ...
              <div style={{
                ...
                <div style={{
                  position: "absolute", top: "4px", bottom: "4px",
                  left: cutType === "contour" ? "4px" : "calc(50% + 2px)",
                  width: "calc(50% - 6px)",
                  background: `linear-gradient(135deg,${ACCENT}cc,${ACCENT})`,
                  borderRadius: "8px", transition: "left 0.28s cubic-bezier(0.34,1.56,0.64,1)",
                  boxShadow: `0 0 14px rgba(${ACCENT_RGB},0.4)`,
                }} />
                {([
                  { id: "contour" as CutType, label: "Contour Cut", sub: "Shape-cut" },
                  { id: "kiss" as CutType, label: "Kiss Cut", sub: "On backing" },
                ]).map((ct) => (
                  <button key={ct.id} onClick={() => setCutType(ct.id)} style={{
                    position: "relative", zIndex: 1, padding: "8px 4px", borderRadius: "8px",
                    cursor: "pointer", border: "none", background: "transparent", textAlign: "center",
                  }}>
                    <div style={{ fontSize: "10px", fontWeight: 700, fontFamily: "var(--font-orbitron)", color: cutType === ct.id ? "white" : "rgba(255,255,255,0.32)" }}>
                      {ct.label}
                    </div>
                    <div style={{ fontSize: "8px", marginTop: "2px", color: cutType === ct.id ? "rgba(255,255,255,0.65)" : "rgba(255,255,255,0.18)" }}>
                      {ct.sub}
                    </div>
                  </button>
                ))}
                ...
                {SHAPES.map((s) => {
                  ...
                  return (
                    <button key={s.id} className="sc-opt" onClick={() => setShape(s.id)} style={{
                      ...
                      padding: "8px 6px 6px", borderRadius: "12px", cursor: "pointer",
                      border: `1px solid ${sel ? `rgba(${ACCENT_RGB},0.55)` : "rgba(255,255,255,0.07)"}`,
                      background: sel ? `rgba(${ACCENT_RGB},0.1)` : "rgba(255,255,255,0.03)",
                      boxShadow: sel ? `0 0 16px rgba(${ACCENT_RGB},0.2)` : "none",
                    }}>
                      <div style={{ position: "relative", width: "48px", height: "48px" }}>
                        <Image src={shapeImage(material, s.id, cutType)} alt={s.label} fill className="object-contain" unoptimized />
                      </div>
                ...
                {SIZE_PRESETS.map((s) => {
                  ...
                  return (
                    <button key={s.id} className="sc-opt" onClick={() => setSizeId(s.id)} style={{
                      ...
                      border: `1px solid ${sel ? `rgba(${ACCENT_RGB},0.55)` : "rgba(255,255,255,0.07)"}`,
                      background: sel ? `rgba(${ACCENT_RGB},0.1)` : "rgba(255,255,255,0.03)",
                      boxShadow: sel ? `0 0 18px rgba(${ACCENT_RGB},0.2)` : "none",
                    }}>
                      <span style={{ fontSize: "12px", fontWeight: 700, fontFamily: "var(--font-orbitron)", color: sel ? "white" : "rgba(255,255,255,0.5)" }}>
                        {s.label}
                      </span>
                    </button>
                  );
                ...
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <input
                      ...
                      placeholder="W (in)" className="sc-input"
                    ...
                    <input
                      ...
                      placeholder="H (in)" className="sc-input"
                      style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white", fontSize: "13px", padding: "8px 12px", borderRadius: "10px" }}
                    />
                  </div>

                  <div>
                    <p style={{ fontSize: "9px", color: "rgba(255,255,255,0.35)", fontFamily: "var(--font-orbitron)", letterSpacing: "0.08em", marginBottom: "6px" }}>
                      OTHER POPULAR SIZES:
                    </p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                      {POPULAR_SIZES.map((p) => {
                        ...
                        return (
                          <button key={p.key} className="sc-opt" onClick={() => { setCustomW(String(p.w)); setCustomH(String(p.h)); }} style={{
            ...
            <div className="sc-card rounded-[20px] p-5 flex flex-col gap-3 relative overflow-hidden"
              ...
              <div className="flex flex-col gap-2">
                {([
                  { id: "gloss" as Finish, label: "Gloss", sub: "Bold shine. Vibrant colors." },
                  { id: "matte" as Finish, label: "Matte", sub: "Smooth finish. No glare." },
                ]).map((f) => {
                  const sel = finish === f.id;
                  return (
                    <button key={f.id} className="sc-opt" onClick={() => setFinish(f.id)} style={{
            ...
            <div className="sc-card rounded-[20px] p-5 flex flex-col gap-2 relative overflow-hidden"
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "3px", flex: 1 }}>
                {QTY_PRESETS.map((qty) => {
                  const { perUnit: pu, total: tot, savings: sav } = priceFor(qty);
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
                        ...
                        {sav > 0 && (
                          <span style={{
                            ...
                            background: active ? `rgba(${ACCENT_RGB},0.3)` : "rgba(34,197,94,0.14)",
                ...
                <button className="sc-opt" onClick={() => setSelectedTierQty("custom")} style={{
                  ...
                  <span style={{ fontFamily: "var(--font-orbitron)", fontSize: "11px", fontWeight: 700, color: selectedTierQty === "custom" ? "white" : "rgba(255,255,255,0.45)" }}>
                    Custom
                  </span>
                ...
                {selectedTierQty === "custom" && (
                  <input
                    type="number" min={MIN_QTY} max="50000" step="1"
                    value={customQtyInput} onChange={(e) => setCustomQtyInput(e.target.value)}
                    placeholder={`Enter quantity (min ${MIN_QTY})`} className="sc-input"
              ...
              <div style={{
                ...
                background: `linear-gradient(135deg,rgba(${ACCENT_RGB},0.2) 0%,rgba(${ACCENT_RGB},0.1) 100%)`,
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <p style={{ fontFamily: "var(--font-orbitron)", fontSize: "1.125rem", fontWeight: 900, color: "white" }}>
                      {activeTotal > 0 ? fmt(activeTotal) : dims === null ? "Enter size" : "—"}
                    </p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    ...
                    <p style={{ fontSize: "0.875rem", fontWeight: 700, color: `rgba(${ACCENT_RGB},0.9)` }}>
              ...
              {proofResult ? (
                ...
                <>
                  <div className="sc-upload-icon" style={{
                    ...
                    background: `rgba(${ACCENT_RGB},0.1)`, border: `1px solid rgba(${ACCENT_RGB},0.2)`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                  }}>
            ...
            <div style={{ marginTop: "1rem", paddingBottom: "1rem" }}>
              <textarea
                ...
                style={{
                  ...
                  color: "white", fontSize: "13px", padding: "14px 16px", resize: "none", lineHeight: 1.7,
                }}
              />
            </div>
          </div>
            ...
            <button
              ...
              style={{
                ...
                background: file && canProceed
                  ? `linear-gradient(135deg,${ACCENT} 0%,rgba(${ACCENT_RGB},0.72) 100%)`
